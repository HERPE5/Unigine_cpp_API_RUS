// Полнофункциональный контроллер персонажа от первого лица на основе коллизии капсулы.
// Поддерживает ходьбу, бег, приседание, прыжки, автоматическое преодоление ступеней,
// обработку наклонных поверхностей и взаимодействие с физическими объектами. Использует
// коллизии по поверхности форм для определения земли и собственную обработку коллизий.
...
// Контроллер персонажа от первого лица с физикой капсулы, режимами передвижения
// и настраиваемыми привязками ввода для клавиатуры и геймпада.
class FirstPersonController : public Unigine::ComponentBase
{

	// Список названий клавиш через запятую для выпадающих списков PROP_PARAM Switch
...
#pragma region EditorParameters
	...
	// Какой аналоговый стик использовать для передвижения или камеры
	...
	// Стратегия создания камеры
	enum CameraMode
	{
		NONE = 0,				// Без управления камерой
		CREATE_AUTOMATICALLY,	// Контроллер создаёт собственный PlayerDummy
		USE_EXTERNAL			// Использовать камеру, назначенную в редакторе
	};
	...
	struct DebugColors : Unigine::ComponentStruct
	{
		...
		PROP_PARAM(Color, player_direction, Unigine::Math::vec4(1.0f, 1.0f, 0.0f, 1.0f), "Player Direction", "Player direction"); // жёлтый
		PROP_PARAM(Color, camera_color, Unigine::Math::vec4(0.0f, 1.0f, 1.0f, 1.0f), "Camera Color", "Camera color"); // голубой
		...
		PROP_PARAM(Color, up_pass_contacts, Unigine::Math::vec4(0.0f, 1.0f, 1.0f, 1.0f), "Up Pass Contacts", "Up pass contacts"); // голубой
		PROP_PARAM(Color, side_pass_contacts, Unigine::Math::vec4(0.87f, 0.0f, 1.0f, 1.0f), "Side Pass Contacts", "Side pass contacts"); // пурпурный
		...
		PROP_PARAM(Color, average_speed, Unigine::Math::vec4(1.0f, 1.0f, 0.0f, 1.0f), "Average Speed", "Average speed"); // жёлтый
...
private:
	void init();			// Создаёт физическое тело, камеру, предварительно вычисляет углы
	void update();			// Основной цикл: ввод, подшаги физики, приседание, камера
	void update_physics();	// Физика с фиксированной частотой: применяет импульсы к твёрдым телам

	void update_move_directions(float ifps);	// Считывает ввод с клавиатуры/геймпада
	void check_move_and_stair();				// Трассировка лучей для проходимых поверхностей и ступеней
	void update_velocity(float ifps, float update_part);	// Применяет ускорение, затухание, гравитацию
	void update_collisions(float ifps);			// Обрабатывает контакты капсулы с миром
	void try_move_up(float ifps);				// Автоступени: подъём над препятствием
	void move_side(float ifps);					// Проход горизонтальных коллизий
	void try_move_down(float ifps);				// Автоступени: возврат обратно на землю
	void update_crouch(float ifps);				// Конечный автомат плавного перехода приседания
	void update_camera();						// Позиционирует камеру со смещениями и поворотом
	void swap_interpolation_direction(Unigine::Math::Scalar start_height, Unigine::Math::Scalar end_height);	// Меняет направление интерполяции приседания
	void update_player_height(Unigine::Math::Scalar height);	// Изменяет размер капсулы и смещение камеры
...
private:
	...
	bool is_initialized = false;	// True после успешного init()

	Unigine::Math::vec3 additional_camera_offset = Unigine::Math::vec3_zero;	// Дополнительное смещение камеры
	Unigine::Math::quat additional_camera_rotation = Unigine::Math::quat_identity;	// Дополнительный поворот камеры

	bool is_ground = false;			// Стоит на проходимой поверхности
	bool is_ceiling = false;		// Голова касается потолка
	bool is_crouch = false;			// В данный момент приседает
	bool is_horizontal_frozen = false;	// Скорость ниже физического порога

	// Векторы базиса наклонной поверхности для движения, выровненного по поверхности
	...
	Unigine::Math::Vec3 horizontal_velocity = Unigine::Math::Vec3_zero; // Скорость в текущем базисе наклонной поверхности
	float vertical_velocity = 0.0f;	// Вертикальная скорость (гравитация, прыжок)

	#pragma endregion

	// Пороги и смещения для передвижения
	const Unigine::Math::vec2 forward { 0, 1 };	// Направление ввода "вперёд"
	const Unigine::Math::vec2 right { 1, 0 };	// Направление ввода "вправо"
	const float skin_width_offset = 0.05f;		// Оболочка коллизии для предотвращения проникновения
	const float auto_stepping_speed_threshold = 0.1f;	// Минимальная скорость для срабатывания автоступеней
	const float large_epsilon = 0.001f;			// Допуск для сравнения наклонных поверхностей

	float player_ifps = 1.0f;	// Обратная величина частоты обновления игрока

	Unigine::InputGamePadPtr game_pad;	// Подключённый геймпад (если есть)

	Unigine::BodyDummyPtr body;				// Физическое тело для коллизий
	Unigine::ShapeCapsulePtr shape;			// Капсула коллизии игрока
	Unigine::ShapeCylinderPtr interaction_shape;	// Немного большая форма для толкания объектов

	Unigine::Vector<Unigine::ShapeContactPtr> contacts;	// Буфер контактов коллизий
	bool is_available_side_move = false;	// Луч обнаружил проходимую поверхность впереди
	bool is_available_stair = false;		// Луч обнаружил поверхность ступени впереди
	bool has_bottom_contacts = false;		// Есть ли контакты на нижней полусфере
	bool is_heavy_contacts = false;			// Количество контактов превышает порог

	float camera_vertical_angle = 0.0f;		// Угол тангажа (взгляд вверх/вниз)
	float camera_horizontal_angle = 0.0f;	// Угол рыскания (взгляд влево/вправо)
	Unigine::Math::vec3 camera_crouch_offset = Unigine::Math::vec3_zero;	// Корректировка высоты камеры при приседании

	Unigine::Math::vec2 horizontal_move_direction = Unigine::Math::vec2_zero;	// Направление ввода с клавиш/стика
	float vertical_move_direction = 0.0f;	// Импульс прыжка в этом кадре

	float max_air_speed = 0.0f;	// Ограничение скорости в воздухе

	bool used_auto_stepping = false;		// Автоступень применена в этом кадре
	Unigine::Math::Scalar last_step_height = 0.0f;	// Высота последней автоступени

	// Предварительно вычисленные косинусы пороговых углов
	...
	Unigine::Math::Mat4 world_transform = Unigine::Math::Mat4_identity;	// Текущая трансформация игрока

	Unigine::WorldIntersectionNormalPtr normal_intersection;	// Результат трассировки луча с нормалью

	// Фазы конечного автомата приседания
	enum CrouchPhase
	{
		STAND = 0,		// Полная высота
		MOVE_DOWN,		// Переход в присед
		CROUCH,			// Присед
		MOVE_UP			// Переход в положение стоя
	};

	// Состояние интерполяции для плавных переходов приседания
	struct CrouchState
	{
		CrouchPhase phase = CrouchPhase::STAND;
		float current_time = 0.0f;				// Оставшееся время перехода
		Unigine::Math::Scalar current_height;	// Текущая высота капсулы
		Unigine::Math::Scalar start_height;		// Высота в начале перехода
		Unigine::Math::Scalar end_height;		// Целевая высота
	};

	CrouchState crouch_state;	// Текущее состояние перехода приседания
