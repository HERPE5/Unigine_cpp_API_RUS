// Обрабатывает прямоугольный выбор в стиле RTS: перетаскивание создаёт прямоугольник выбора,
// отпускание выбирает все юниты внутри фрустума. Одиночный клик выбирает
// объект под курсором. Выбранные объекты отображают круги выбора.
...
// Возвращает центр совместной ограничивающей сферы всех выбранных объектов.
...
// Возвращает радиус совместной ограничивающей сферы (масштабированный в 4 раза для кадрирования камерой).
...
// Возвращает true, если в данный момент выбраны какие-либо объекты.
bool Selection::hasSelection()
{
	return selected_objects.size() != 0;
}

// Обрабатывает ввод выбора: нажатие ЛКМ начинает перетаскивание, отпускание ЛКМ завершает и запрашивает объекты.
// Отрисовывает прямоугольник выбора во время перетаскивания.
void Selection::update()
{
	// Работает только когда консоль неактивна
	if (!Console::isActive())
	{
		Visualizer::setEnabled(true);

		// Если левая кнопка мыши нажата, сохраняем координаты курсора и начинаем выбор
		...
		// Отрисовываем прямоугольник выбора
		if (is_selection)
		{
		...
		// При отпускании левой кнопки мыши запрашиваем объекты в прямоугольнике выбора
		if (Input::isMouseButtonUp(Input::MOUSE_BUTTON_LEFT))
		{
			PlayerPtr camera = Game::getPlayer();

			// Снимаем выбор с ранее выбранных объектов
			...
			// Выбор завершён - строим фрустум из экранного прямоугольника
			is_selection = false;

			// Строим фрустум из прямоугольника выбора для пространственного запроса.
		// Фрустум простирается от камеры через прямоугольник в экранном пространстве.
			...
			// Находим все объекты, чьи границы пересекаются с фрустумом выбора
			...
			// Отмечаем выбранные объекты и отфильтровываем невыбираемые
...
// Вычисляет совместную ограничивающую сферу всех выбранных объектов.
