...
// Окно GUI со слайдером масштаба и кнопками целей создаётся.
void ZoomSample::init()
{
	...
	// Callback слайдера обновляет масштаб и обновляет все метки параметров
	...
	// Метки отображают текущие настройки камеры, затронутые масштабированием
	...
	// Кнопки целей позволяют быстро ориентировать камеру на предопределённые объекты
...
// Требуемые компоненты и узлы проверяются.
