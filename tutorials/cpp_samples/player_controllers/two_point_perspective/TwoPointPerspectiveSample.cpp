// UI примера для переключения режима двухточечной перспективы. Включает стиль архитектурной
// визуализации, где вертикальные линии остаются вертикальными независимо от
// угла наклона камеры.
...
{
	// Получаем контроллер камеры от игрока
	...
	// Создаём окно UI с элементами управления
	init_gui();
}
...
void TwoPointPerspectiveSample::init_gui()
{
	...
	// Чекбокс для включения/отключения режима двухточечной перспективы
...
{
	// Переключаем режим сдвига камеры при изменении состояния чекбокса
