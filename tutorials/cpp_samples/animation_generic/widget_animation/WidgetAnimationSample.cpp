...
// Этот компонент демонстрирует анимацию виджетов с помощью AnimationObjectRuntime.
// В отличие от объектов анимации узлов/материалов, объекты runtime могут нацеливаться на любой объект C++
// (виджеты, пользовательские классы) путём прямой привязки к указателям объектов во время выполнения.
// Показана анимация позиции виджета, размера шрифта и цветовых свойств.
...
private:
	void init()
	{
		// Создаём треки и воспроизведение
		create_animations();
		...
		// Привязка во время выполнения: соединяем объекты анимации с фактическими экземплярами виджетов
		// Это должно быть сделано во время выполнения, так как виджеты создаются динамически
		{
			...
			// Привязываем первый объект анимации к метке Player 0
			anim_obj = checked_ptr_cast<AnimationObjectRuntime>(track->getObject(0));
			bind = anim_obj->getBind();
			bind->setWidget(player_0_label);	// прямая привязка по указателю
			anim_obj->setBind(bind);

			// Привязываем второй объект анимации к метке Player 1
			anim_obj = checked_ptr_cast<AnimationObjectRuntime>(track->getObject(1));
			bind = anim_obj->getBind();
			bind->setWidget(player_1_label);
			anim_obj->setBind(bind);

			// Привязываем третий объект анимации к метке "vs"
	...
	// Создаём трек анимации для заголовка "Player 0 vs Player 1"
	void create_animations()
	{
		AnimationTrackPtr track = AnimationTrack::create();

		// AnimationObjectRuntime - универсальные runtime-объекты для виджетов и пользовательских целей
		// В отличие от объектов узла/материала, они не используют привязку на основе ресурсов
		...
		// Появление метки Player 0 слева
		auto player_0_pos_modifier = AnimationModifierInt::create("widget.position_x");
		player_0_pos_modifier->addValue(0.0f, 0);		// за пределами экрана слева
		player_0_pos_modifier->addValue(0.25f, 250);	// появление
		player_0_pos_modifier->addValue(2.75f, 250);	// удержание
		player_0_pos_modifier->addValue(3.0f, 0);		// исчезновение
		player_0_pos_modifier->addValue(5.0f, 0);
		track->addObjectModifier(player_0_obj, player_0_pos_modifier);

		// Появление метки Player 1 справа
		auto player_1_pos_modifier = AnimationModifierInt::create("widget.position_x");
		player_1_pos_modifier->addValue(0.0f, 1225);	// за пределами экрана справа
		player_1_pos_modifier->addValue(0.25f, 975);	// появление
		player_1_pos_modifier->addValue(2.75f, 975);
		player_1_pos_modifier->addValue(3.0f, 1225);	// исчезновение
		player_1_pos_modifier->addValue(5.0f, 1225);
		track->addObjectModifier(player_1_obj, player_1_pos_modifier);

		// Появление/исчезновение обеих меток игроков (альфа-компонент)
		...
		// Метка "vs": циклическая анимация цвета (голубой -> жёлтый -> голубой)
		...
		vs_color_modifier->addValue(0.3f, vec4(0.0f, 1.0f, 1.0f, 1.0f));	// появление
		vs_color_modifier->addValue(1.5f, vec4(1.0f, 1.0f, 0.0f, 1.0f));	// переход к жёлтому
		vs_color_modifier->addValue(2.70f, vec4(0.0f, 1.0f, 1.0f, 1.0f));	// возврат к голубому
		vs_color_modifier->addValue(2.75f, vec4(0.0f, 1.0f, 1.0f, 0.001f));	// исчезновение
