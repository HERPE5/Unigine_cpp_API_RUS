// Демонстрирует SplineGraph для создания и прохождения плавных кривых в 3D-пространстве.
// Точки и сегменты создаются процедурно, затем визуализируются с векторами касательных.
// Объект-меш следует по пути сплайна с помощью calcSegmentPoint() для плавной анимации.
...
namespace
{
	// Генерирует случайный vec3 в пределах min/max
	...
	// Генерирует случайный vec3 в симметричных пределах
...
public:
	...
	// Файл меша, используемый для движущегося объекта
...
private:
	void init()
	{
		...
		// Создаём пустой контейнер сплайн-графа
		spline_graph = SplineGraph::create();
		// Создаём меш, который будет следовать по пути сплайна
	...
	void generate_graph()
	{
		...
		// Инициализируем случайное зерно из текущего времени
		Game::setSeed(int(Time::get()));

		// Создаём случайные контрольные точки в пределах заданного объёма
		for (int i = 0; i < num_points; i += 1)
			spline_graph->addPoint(Vec3(::random_vec3(size)) + offset);

		// Соединяем точки кубическими сегментами Безье (замкнутый контур)
		for (int i = 0; i < num_points; i += 1)
		{
			// Окно из 4 точек для вычисления касательных в стиле Catmull-Rom
			int index_window[4] = {
			...
			// Вычисляем плавные касательные: направление от предыдущей к следующей точке
			...
			// Создаём сегмент с касательными и векторами вверх
	...
	void render_graph()
	{
		const int num_segments = 32;  // Уровень подразбиения для визуализации

		// Рисуем контрольные точки в виде белых точек
		...
		// Рисуем каждый сегмент сплайна с векторами касательных
		for (int i = 0; i < spline_graph->getNumSegments(); i++)
		{
			vec4 tangent_color = vec4(vec3(89, 204, 249) / 255.f, 1.f);

			// Получаем конечные точки сегмента и их касательные
			...
			// Визуализируем векторы касательных в контрольных точках
			...
			// Рисуем кривую как серию отрезков линий
			for (int j = 0; j < num_segments; j++)
			{
				// calcSegmentPoint() возвращает интерполированную позицию вдоль сегмента (t: 0-1)
	...
	void move_box_along_graph()
	{
		...
		// Вычисляем позицию вдоль всего сплайна (циклически проходит по сегментам)
		float t = time - Math::floor(time / spline_graph->getNumSegments()) * spline_graph->getNumSegments();
		// Извлекаем индекс сегмента и локальный параметр
		int segment_id = int(t);
		t -= float(segment_id);

		// Получаем интерполированную позицию, касательную (направление) и вектор вверх
		...
		// Применяем позицию и ориентацию к движущемуся мешу
	...
	// Процедурный путь сплайна
	SplineGraphPtr spline_graph;
	// Меш, следующий по сплайну
