// UI примера для извлечения G-buffer. Отображает извлечённые текстуры на объектах
// экрана и поддерживает переключение между главной камерой и пользовательской орбитальной
// камерой для захвата G-buffer.
...
// Узлы отображения подключаются к извлечённым текстурам; кнопки UI переключают режимы камеры.
...
// Вычисляются трансформации орбитальной камеры; узел рендерится в режиме пользовательской камеры.
...
// Исходное состояние визуализатора восстанавливается, UI очищается.
...
// Настраивает узел отображения для показа извлечённой текстуры через канал emission (без освещения).
