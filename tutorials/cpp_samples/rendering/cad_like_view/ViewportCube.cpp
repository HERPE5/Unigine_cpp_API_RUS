// Интерактивный виджет 3D-куба ориентации для навигации камерой. Клик по граням,
// рёбрам или углам куба анимирует вращение камеры к этой точке обзора.
// Поддерживает переключение орто/перспектива и показывает направляющие стрелки при выравнивании.
...
namespace
{
	// Параметры UI и отображения куба
	...
	: player(player), ortho_width(ortho_width), ortho_height(ortho_height)
{
	...
	// Гарантируем, что вектор "вверх" игрока выровнен по оси Z для правильной ориентации куба
	if (player->getType() != Node::NODE_DUMMY)
		player->setUp(normalize(vec3(0, 0.00000001, 1)));

	// Загружаем ресурсы куба (иконки, меши, текстуры) из свойства ViewCubeSettings
	...
	// Кнопка переключения проекции (переключение орто/перспектива)
	...
	// Строим канву куба из предоставленного меша. Каждая поверхность меша представляет выбираемый
	// элемент куба: грань (например, +x), ребро (комбинация вроде +x и +y) или угол (+x и +y и +z).
	MeshPtr mesh = Mesh::create(viewcube_mesh_path);
	cube_canvas = WidgetCanvas::create(gui);

	// Создаём канву меша куба и сохраняем позиции/вращения для каждой грани
	...
	// Создаём канву меша стрелок для индикаторов навигации
	...
	// Настраиваем трансформации для куба в режимах ортографии и перспективы
...
// Отображает имя поверхности в вектор направления.
// Имя поверхности может включать несколько токенов осей (например, "px", "py", "pz", "nx", "ny", "nz").
// Результирующие векторы представляют:
//   - грань:    ровно одна ось установлена	(например, (1, 0, 0))
//   - ребро:    две оси установлены			(например, (1, 1, 0))
//   - угол:  три оси установлены			(например, (1, 1, 1))
// См. файл .../data/cpp_samples/systems/cad_like_view/viewcube/viewcube.mesh
...
// Вычисляет вращение камеры, смотрящее в заданном направлении
...
// Переключение между ортографической и перспективной проекцией
...
// Запускает плавное вращение к целевой ориентации
...
// Обновляет текущую анимацию вращения
...
// Основное обновление: обрабатывает рендеринг куба, ввод и взаимодействия
void ViewportCube::update()
{
	...
	// Синхронизируем иконку переключения орто с текущей проекцией
	...
	// Применяем трансформации в зависимости от проекции
	if (is_ortho)
	{
	...
	// Применяем эффект затухания на основе расстояния курсора от центра куба
	// Влияет на кнопку переключения орто и навигационные стрелки
	vec4 color = vec4_one;
	...
	// Подсвечиваем грани куба при наведении
	int cube_polygon = -1;
	...
	// Показываем стрелки только когда камера выровнена по оси
	...
	// Подсвечиваем стрелки при наведении
	...
	// cube_polygon: вращает камеру к выбранной поверхности куба (грань, ребро или угол в зависимости от попадания)
	// arrow_polygon: вращает камеру в направлении, указанном навигационной стрелкой.
	if (!is_hovered)
	{
		return;
	}
