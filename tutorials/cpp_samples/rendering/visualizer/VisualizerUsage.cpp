// Полная демонстрация 2D- и 3D-примитивов Visualizer. Показывает
// все доступные методы рендеринга: точки, линии, треугольники, квады, боксы,
// сферы, капсулы, цилиндры, эллипсы, усечённые пирамиды и ограничивающие объёмы.
...
// Инициализирует компонент VisualizerUsage и проверяет ссылки на узлы
void VisualizerUsage::init()
{
	...
	// Приводим ссылки на узлы к соответствующим типам объектов
	...
	// Проверяем массив узлов-постаментов
...
// Рендерит различные 3D-примитивы визуализации, используя узлы-постаменты как опорные точки
void VisualizerUsage::update3D()
{
	...
	// 3D-биллборд
	current_point = getPostamentPoint(i);
	// Попробуйте использовать setTextureName, чтобы применить текстуру к 3D-биллборду
	...
	// Матрица трансформации равна инвертированной ModelView
	...
	// 3D-сообщение
	current_point = getPostamentPoint(i);
	// Обеспечьте правильное выравнивание сообщения - подробнее см. в документации
...
// Получает мировую позицию из массива узлов-постаментов для правильного размещения визуализации
