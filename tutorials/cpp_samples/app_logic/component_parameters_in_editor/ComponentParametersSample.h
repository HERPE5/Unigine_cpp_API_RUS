...
// Исчерпывающий справочник по объявлениям параметров Component System.
// Демонстрирует все типы PROP_PARAM (скаляры, векторы, ссылки на контент, маски),
// PROP_STRUCT/PROP_ARRAY для сложных данных, и продвинутые возможности на основе args
// (условия видимости, ограничения диапазона, фильтры файлов, подсказки редактора).
// Также показаны все коллбэки жизненного цикла (init, варианты update, physics, swap, shutdown).

// Дополнительную информацию см. на этих страницах документации:
...
public:
	...
	{
		// может иметь любые параметры, как и компонент
	...
	// (примитивный) параметр-массив
	PROP_ARRAY(Int, array_parameter);

	// параметр - массив структур
	...
	// Дополнительную информацию см. в компоненте ComponentParameterArgumentsReference
	// ниже, а также на соответствующей странице документации [1].

	// - type (обязательный)
	// - name (обязательный)
	// - mask type (только для параметров Mask, опциональный)
	// - значение по умолчанию (кроме параметров Struct и Array, опционально для Vec*, Color, Property, Material, Node и Curve2d)
	// - items (только для параметров Switch, обязательный)
	// - title (опциональный)
	// - tooltip (опциональный)
	// - group (опциональный)
	// - args (опциональный)
	...
	// =================== Продвинутые возможности с Args ===================

	// Секции ниже используют аргумент параметра `args` для
	// задания некоторых "продвинутых" атрибутов параметров компонента,
	// таких как диапазон мин/макс для скалярных параметров, фильтры для параметров
	// контента, видимость параметра в редакторе и т.д.

	// Дополнительную информацию см. на соответствующей странице документации [2].
...
// Краткий справочник по порядку аргументов макроса PROP_PARAM.
// Каждый тип параметра имеет свою сигнатуру с опциональными title/tooltip/group/args.
// Полезно для копирования при определении новых параметров компонента.
class ComponentParameterArgumentsReference: public Unigine::ComponentBase
{
	...
	// ============= Аргументы параметров компонента =============
	...
	// ... то же для Int, Toggle, Float, Double, String
	...
	//  ... то же для Vec2, Vec3, Vec4, DVec2, DVec3, DVec4, IVec2, IVec3, IVec4, Color
	...
	// ... то же для Property, Material, Node, Curve2d
	...
	// ------- Задание Title, Tooltip или Group извне -------

	PROP_TITLE("Title");
	PROP_TOOLTIP("Tooltip");
	PROP_PARAM(String, some_parameter); // у этого параметра будут title "Title" и tooltip "Tooltip"

	PROP_GROUP("Group");
	PROP_PARAM(Int, group_member_a); // этот параметр будет членом группы "Group"
	PROP_PARAM(Float, group_member_b); // этот параметр будет членом группы "Group"
	PROP_GROUP(0);
};
