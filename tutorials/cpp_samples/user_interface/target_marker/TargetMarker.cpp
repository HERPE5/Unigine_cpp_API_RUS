// Реализует систему индикации цели за пределами экрана. Когда цель видна,
// спрайт маркера выделяет её позицию. Когда цель вне экрана, спрайт стрелки
// поворачивается, чтобы указать направление на цель с позиционированием по краям экрана.

#include "TargetMarker.h"

REGISTER_COMPONENT(TargetMarker);

using namespace Unigine;
using namespace Math;

// Спрайты загружаются и присоединяются к главному окну, цель и камера проверяются.
void TargetMarker::init()
{
	// Загружаем спрайт стрелки для индикации за пределами экрана
	if (!arrowSprite)
	{
		Log::error("TargetMarker::init(): Source file for the pointer sprite image is not found.");
		return;
	}
	arrow = WidgetSprite::create(arrowSprite.get());
	WindowManager::getMainWindow()->addChild(arrow, Gui::ALIGN_OVERLAP);

	// Загружаем спрайт точечного маркера для выделения на экране
	if (!pointSprite)
	{
		Log::error("TargetMarker::init(): Source file for the marker sprite image is not found.");
		return;
	}

	point = WidgetSprite::create(pointSprite.get());
	WindowManager::getMainWindow()->addChild(point, Gui::ALIGN_OVERLAP);

	// Проверяем ссылку на узел цели
	if (!targetNode)
	{
		Log::error("TargetMarker::init(): No target object specified.");
		return;
	}
	target = targetNode.get();

	// Компонент должен быть прикреплён к узлу Player для расчётов проекции
	camera = checked_ptr_cast<Player>(node);
	if (!camera)
	{
		Log::error("TargetMarker::init(): Camera is not valid.");
		return;
	}
}

// Позиция цели проецируется на экран, отображается соответствующий индикатор.
void TargetMarker::update()
{
	if (!arrow || !point || !target || !camera)
		return;

	// Кэшируем размеры спрайта для расчётов позиционирования стрелки
	arrow_height = arrow->getLayerHeight(0);
	arrow_width = arrow->getLayerWidth(0);
	arrow_half_height = arrow_height / 2;
	arrow_half_width = arrow_width / 2;

	point_width = point->getLayerWidth(0);
	point_height = point->getLayerHeight(0);
	// Смещаем маркер на настраиваемую точку опоры, масштабированную под DPI дисплея
	mat4 translation = mat4(translate(Vec3(-point_width * pointPivot.get().x, -point_height * pointPivot.get().y, 0.0f) * WindowManager::getMainWindow()->getDpiScale()));
	point->setTransform(translation);

	// Начинаем со скрытыми обоими; видимость определяется ниже на основе позиции цели
	arrow->setHidden(true);
	point->setHidden(true);

	// Размер окна
	int width = WindowManager::getMainWindow()->getClientSize().x;
	int height = WindowManager::getMainWindow()->getClientSize().y;
	int halfWidth = width / 2;
	int halfHeight = height / 2;

	// Точка цели в экранном пространстве
	int x = 0;
	int y = 0;

	// Вектор направления от камеры к центру цели
	vec3 targetDirection(target->getWorldBoundBox().getCenter() - camera->getWorldPosition());

	// Отрицательное скалярное произведение означает, что цель за плоскостью обзора камеры
	bool behind = dot(camera->getWorldDirection(Unigine::Math::AXIS_NZ), targetDirection) < 0;

	// Находим проекцию точки цели на плоскость экранного пространства и меняем систему координат на центр экрана
	if (!behind)
	{
		camera->getScreenPosition(x, y, target->getWorldBoundBox().getCenter(), width, height);

		x -= halfWidth;
		y -= halfHeight;
		y *= -1;
	}
	// Если цель вне видимости, вычисляем позицию в экранном пространстве относительно отражённой позиции цели
	else
	{
		Vec3 inverseScreenPlaneNormal(camera->getViewDirection() * -1);
		Vec3 relativeToCameraTargetPosition = target->getWorldBoundBox().getCenter() - camera->getWorldPosition();

		// Ортогональная проекция relativeToCameraTargetPosition на inverseScreenPlaneNormal
		Vec3 orthoProjectionTarget = inverseScreenPlaneNormal * dot(relativeToCameraTargetPosition, inverseScreenPlaneNormal);
		Vec3 reflectedTargetPosition = (relativeToCameraTargetPosition - orthoProjectionTarget * 2) + camera->getWorldPosition();

		camera->getScreenPosition(x, y, reflectedTargetPosition, width, height);

		x -= halfWidth;
		y -= halfHeight;
		// Когда цель вне видимости, стрелка всегда находится в нижней части экрана
		if (y > 0)
			y *= -1;
	}

	// Если цель внутри поля зрения, выделяем её маркером
	if (!behind && x >= -halfWidth && x <= halfWidth && y >= -halfHeight && y <= halfHeight)
	{
		point->setHidden(false);
		point->setPosition(x + halfWidth, -y + halfHeight);
	}
	// Если цель вне видимости, показываем стрелку, указывающую на неё
	else
	{
		// Вычисляем точку опоры для стрелки
		int point_x = 0;
		int point_y = 0;
		getIntersectionWithRect(point_x, point_y, x, y, halfWidth, halfHeight);
		float angle = 0.0f;

		// Задаём позицию и поворот спрайта стрелки (позиция спрайта относительна его верхнему левому углу)
		// Стрелка находится в углу
		if (halfHeight - Math::abs(point_y) <= arrow_half_height && halfWidth - Math::abs(point_x) <= arrow_half_width)
		{
			float dx, dy;

			if (point_y > 0)
				dy = (float)(point_y - (halfHeight - arrow_half_width));
			else
				dy = (float)(point_y + (halfHeight - arrow_half_width));

			if (point_x > 0)
				dx = (float)(point_x - (halfWidth - arrow_half_width));
			else
				dx = (float)(point_x + (halfWidth - arrow_half_width));

			angle = -Math::atan2(dy, dx) * Consts::RAD2DEG;

			if (point_x > 0)
				point_x = halfWidth - arrow_width;
			else
				point_x = -halfWidth;

			if (point_y > 0)
				point_y = halfHeight;
			else
				point_y = -halfHeight + arrow_height;
		}
		// Стрелка указывает вверх
		else if (point_y == halfHeight)
		{
			point_x -= arrow_half_width;
			angle = -90;
		}
		// Стрелка указывает вниз
		else if (point_y == -halfHeight)
		{
			point_y += arrow_height;
			point_x -= arrow_half_width;
			angle = 90;
		}
		// Стрелка указывает влево
		else if (point_x == -halfWidth)
		{
			point_y += arrow_half_height;
			angle = 180;
		}
		// Стрелка указывает вправо
		else if (point_x == halfWidth)
		{
			point_x -= arrow_width;
			point_y += arrow_half_height;
			angle = 0;
		}

		arrow->setHidden(false);
		// Преобразуем из координат с центром в начале обратно в координаты с началом в верхнем левом углу для позиционирования виджета
		arrow->setPosition(point_x + halfWidth, -point_y + halfHeight);

		// Вращаем стрелку вокруг её центра: переносим в начало координат, поворачиваем, переносим обратно
		mat4 rotation(
			mat4(translate(Vec3(arrow_half_width * 1.0f, arrow_half_height * 1.0f, 0.0f) * WindowManager::getMainWindow()->getDpiScale())) *
			rotate(quat(vec3_up, angle)) *
			mat4(translate(Vec3(-arrow_half_width * 1.0f, -arrow_half_height * 1.0f, 0.0f) * WindowManager::getMainWindow()->getDpiScale()))
		);

		arrow->setLayerTransform(0, rotation);
	}
}

// Виджеты спрайтов освобождаются.
void TargetMarker::shutdown()
{
	arrow.deleteLater();
	point.deleteLater();
}

// Вычисляет точку, где луч от центра экрана пересекает прямоугольник границ экрана.
// Используется для ограничения позиции индикатора-стрелки краями экрана, когда цель вне экрана.
void TargetMarker::getIntersectionWithRect(int& x, int& y, int vec_x, int vec_y, int half_width, int half_height)
{
	// Верхняя половина экрана (положительный Y в координатах с центром в начале)
	if (vec_y >= 0)
	{
		// горизонтальный луч - пересекает только левый или правый край
		if (vec_y == 0)
		{
			if (vec_x > 0)
			{
				x = half_width;
				y = 0;
			}
			else
			{
				x = -half_width;
				y = 0;
			}

			return;
		}

		// Сначала пробуем пересечение с верхним краем, используя подобные треугольники
		x = (int)(half_height * ((float)vec_x / (float)vec_y));
		y = half_height;

		// Если точка пересечения в пределах ширины экрана, верхний край ближайший
		if (x >= -half_width && x <= half_width)
			return;

		// Иначе пересекаем с левым или правым краем
		if (vec_x >= 0)
		{
			if (vec_x == 0)
			{
				x = 0;
				y = half_height;

				return;
			}

			// Пересечение с правым краем
			x = half_width;
			y = (int)(half_width * ((float)vec_y / (float)vec_x));

			return;
		}
		else
		{
			// Пересечение с левым краем
			x = -half_width;
			y = (int)(-half_width * ((float)vec_y / (float)vec_x));

			return;
		}
	}
	// Нижняя половина экрана (отрицательный Y в координатах с центром в начале)
	else
	{
		// Сначала пробуем пересечение с нижним краем
		x = (int)(-half_height * ((float)vec_x / (float)vec_y));
		y = -half_height;

		// Если в пределах ширины экрана, нижний край - ближайшее пересечение
		if (x >= -half_width && x <= half_width)
			return;

		// Иначе пересекаем с левым или правым краем
		if (vec_x >= 0)
		{
			if (vec_x == 0)
			{
				x = 0;
				y = -half_height;

				return;
			}

			// Пересечение с правым краем
			x = half_width;
			y = (int)(half_width * ((float)vec_y / (float)vec_x));

			return;
		}
		else
		{
			// Пересечение с левым краем
			x = -half_width;
			y = (int)(-half_width * ((float)vec_y / (float)vec_x));

			return;
		}
	}
}
