// UI примера для компонента Manipulators. Предоставляет чекбоксы для включения/отключения
// отдельных осей для операций перемещения, поворота и масштабирования, а также кнопки-переключатели
// для смены мирового и локального базиса координат для виджетов манипулятора.
...
// Создаётся UI панели управления с чекбоксами и кнопками для настроек манипулятора.
void ManipulatorsSample::init()
{
	// Сохраняем дескриптор мыши для восстановления при завершении
	previous_handle = Input::getMouseHandle();

	// Получаем ссылку на компонент Manipulators на том же узле
	...
	// По умолчанию мировой режим базиса
...
// Дескриптор мыши восстанавливается, и окно UI освобождается.
...
// Callback-функции чекбоксов осей поворота - переключают доступность оси на манипуляторе поворота.
...
// Callback-функции чекбоксов осей перемещения - переключают доступность оси на манипуляторе перемещения.
...
// Callback-функции чекбоксов осей масштаба - переключают доступность оси на манипуляторе масштаба.
...
// Кнопка локального базиса переключает манипулятор на локальную систему координат объекта.
void ManipulatorsSample::local_basis_button_callback()
{
	is_local_basis = local_basis_button->isToggled();
	// Обеспечиваем взаимное исключение между кнопками-переключателями
	world_basis_button->setToggled(!is_local_basis);
	component->setLocalBasis(is_local_basis);
}

// Кнопка мирового базиса переключает манипулятор на мировую систему координат.
