...
// Виджет иконки создаётся с включённым режимом переключения, подключается callback клика.
void Icon::init()
{
	...
	// Включаем поведение переключения для смены состояний вкл/выкл
	widget_icon->setToggleable(true);

	// Callback-лямбда отображает состояние переключения при клике
