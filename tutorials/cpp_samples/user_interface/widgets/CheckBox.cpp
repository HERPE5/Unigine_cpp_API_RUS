// Демонстрирует создание WidgetCheckBox и обработку событий. Изменение состояния
// чекбокса вызывает callback EventChanged, который отображает текущее состояние
// отметки в оверлее консоли.
...
// Виджет чекбокса создаётся, подключается callback изменения состояния.
void CheckBox::init()
{
	...
	// Callback-лямбда отображает состояние отметки при переключении
