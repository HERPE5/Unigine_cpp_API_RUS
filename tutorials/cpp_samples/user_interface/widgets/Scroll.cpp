// Демонстрирует WidgetScroll как самостоятельный элемент управления полосой прокрутки. Ориентация
// полосы прокрутки устанавливается горизонтальной, а изменения значения вызывают callback
// EventChanged, который показывает текущую позицию прокрутки в оверлее консоли.

#include "Scroll.h"
#include <UnigineConsole.h>

REGISTER_COMPONENT(Scroll);

using namespace Unigine;
using namespace Math;

// Виджет горизонтальной полосы прокрутки создаётся с callback изменения значения.
void Scroll::init()
{
	gui = WindowManager::getMainWindow()->getGui();

	widget_scroll = WidgetScroll::create(gui);

	gui->addChild(widget_scroll, Gui::ALIGN_OVERLAP | Gui::ALIGN_BACKGROUND);

	widget_scroll->setPosition(position.get().x, position.get().y);
	// Задаём горизонтальную ориентацию (0 = горизонтальная, 1 = вертикальная)
	widget_scroll->setOrientation(0);

	// Callback-лямбда отображает текущее значение прокрутки при изменении
	widget_scroll->getEventChanged().connect(*this, [this]() {
		String msg = String("Scroll: ") + String::itoa(widget_scroll->getValue());
		Console::onscreenMessageLine(msg.get());
		});

	Console::setOnscreen(true);
}

// Виджет удаляется из GUI, ресурсы освобождаются.
void Scroll::shutdown()
{
	if (gui)
	{
		gui->removeChild(widget_scroll);

		widget_scroll.deleteLater();
	}

	Console::setOnscreen(false);
}
