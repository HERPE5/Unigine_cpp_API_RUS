// UI примера для настройки параметров движения (скорость, угловая скорость),
// общих для всех трёх вариантов движения, плюс кнопка сброса.
...
// UI создаётся с ползунками скорости и кнопкой сброса.
void SimpleMovementInfo::init()
{
	...
	// Ползунок угловой скорости обновляет все три варианта
	...
	// Кнопка сброса возвращает все узлы в исходные позиции
...
// Окно UI закрывается при завершении работы.
void SimpleMovementInfo::shutdown()
{
	sample_description_window.shutdown();
}

// Ссылки на компоненты кэшируются; начальные трансформации сохраняются для сброса.
void SimpleMovementInfo::init_components()
{
	// Получаем ссылки на компоненты из назначенных узлов
	...
	// Проверяем и сохраняем начальные трансформации для каждого варианта
	if (!simple_movement)
	{
