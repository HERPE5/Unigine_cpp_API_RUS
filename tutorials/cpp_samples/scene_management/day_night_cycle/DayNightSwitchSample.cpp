...
// UI примера для системы цикла день/ночь. Предоставляет ползунки для управления временем,
// порогового угла зенита, утренних/вечерних границ и масштаба времени. Использует Visualizer
// для рисования отладочных векторов, показывающих направление солнца относительно вертикальной оси.
REGISTER_COMPONENT(DayNightSwitchSample);

// Создаются виджеты UI и подключаются к SunController и DayNightSwitcher.
void DayNightSwitchSample::init()
{
	...
	// Обновляем ползунок текущим временем, приходящим от контроллера солнца
	sun->getEventOnTimeChanged().connect(*this, [this](float time) {
		MUTE_EVENT(sun->getEventOnTimeChanged());
		time_slider->setValue(time / 60.0f);
		});
}

// Визуализатор отключается, окно UI освобождается.
...
// Рисуются отладочные векторы: вертикальная ось (красный) и направление солнца (синий).
...
// Время в минутах преобразуется в строку формата "ЧЧ:ММ".
