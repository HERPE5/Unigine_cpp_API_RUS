// Демонстрирует тесты пространственного пересечения с использованием объёмов
// усечённой пирамиды, сферы и бокса. Объекты, пересекающие каждый тип объёма,
// подсвечиваются разными материалами. Использует типы WorldBound для поддержки
// мировых координат двойной точности.
...
// Проверяет пересечения объектов с тремя типами ограничивающих объёмов и подсвечивает совпадения.
...
public:
	...
	// Параметры пересечения усечённой пирамиды (конус обзора)
	...
	// Параметры пересечения бокса, выровненного по осям
...
private:
	// Типы WorldBound работают как в одинарной, так и в двойной точности сборки
	...
	// Отслеживает исходные материалы для восстановления после окончания пересечения
	Unigine::Map<Unigine::ObjectMeshStaticPtr, Unigine::MaterialPtr> changed;
	// Применяет материал подсветки к пересекающимся объектам
	void change_color(const Unigine::Vector<Unigine::NodePtr> &nodes, const Unigine::MaterialPtr &new_material);
	// Рисует отладочную визуализацию всех ограничивающих объёмов
	void render_all_bounds() const;

	// Ссылка на Rotator для управления скоростью через UI
	...
	void update();
	void shutdown();
	// Восстанавливает исходные материалы в конце кадра
	void swap();
};