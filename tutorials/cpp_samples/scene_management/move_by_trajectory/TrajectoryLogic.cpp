// UI примера для демонстрации движения по траектории. Управляет тремя самолётами, использующими
// разные подходы к движению (линейный, сплайновый и файл пути). Позволяет переключаться
// между прикреплёнными камерами и переключать визуализацию пути.
...
// Собираются камеры и инициализируются элементы управления UI.
...
// Создаются элементы UI для скорости, переключения камер и визуализации пути.
void TrajectoryLogic::init_gui()
{
	sample_description_window.createWindow();

	// Ползунок скорости одновременно обновляет все три варианта движения
	...
	// Кнопка переключения камеры циклически переключает все доступные виды
	...
	// Чекбокс переключения визуализации пути
...
// Скорость применяется ко всем трём компонентам движения.
...
// Активная камера переключается на следующего игрока в списке.
...
// Отладочная визуализация пути переключается на всех компонентах движения.
