// Пример ввода с геймпада, управляющий автомобилем с помощью аналоговых стиков и триггеров. Показывает
// отслеживание состояний кнопок (down/pressed/up), значения осей, ввод с триггеров и визуализацию
// тачпада. Поддерживает вибрационную обратную связь с настраиваемыми параметрами.
...
// Порог фильтра ввода применяется к геймпаду.
...
// Задаётся мощность мотора вибрации низкой частоты (от 0.0 до 1.0).
void GamepadInput::setLowFrequency(float f)
{
	low_frequency = clamp(f, 0.0f, 1.0f);
}

// Задаётся мощность мотора вибрации высокой частоты (от 0.0 до 1.0).
void GamepadInput::setHighFrequency(float f)
{
	high_frequency = clamp(f, 0.0f, 1.0f);
}

// Длительность вибрации задаётся в миллисекундах.
...
// Настраиваются параметры физики, создаётся UI, получается геймпад.
void GamepadInput::init()
{
	// Порог заморозки физики снижается, чтобы автомобиль оставался отзывчивым
	...
	// Получаем первый доступный геймпад
...
// Считывается состояние геймпада, обновляется UI, применяется управление автомобилем.
...
// Отслеживаются дельты осей, дельты триггеров и переходы состояний кнопок.
void GamepadInput::update_inputs()
{
	if (!gamepad)
		return;

	// Дельты осей отслеживаются только при движении (нулевые значения игнорируются)
	...
	// Дельты триггеров отслеживаются только при нажатии (положительная дельта)
	...
	// Перебираются все кнопки для отслеживания переходов состояний:
	// - isButtonDown: первый кадр нажатия кнопки
	// - isButtonPressed: удержание (срабатывает каждый кадр при удержании)
	// - isButtonUp: первый кадр отпускания кнопки
	for (int i = 0; i < Input::NUM_GAMEPAD_BUTTONS; i++)
	{
		...
		// Нажатые кнопки отслеживаются в наборе, чтобы отличить первое нажатие от удержания
...
// Визуализируется тач-ввод с тачпада геймпада (например, контроллеров PS4/PS5).
...
// Enum типа устройства преобразуется в удобочитаемую строку.
...
// Enum типа модели геймпада преобразуется в удобочитаемую строку.
...
// Enum кнопки геймпада преобразуется в удобочитаемую строку.
