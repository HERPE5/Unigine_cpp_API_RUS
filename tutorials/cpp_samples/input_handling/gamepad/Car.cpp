// Контроллер автомобиля на основе физики с использованием JointWheel для симуляции колёс. Реализует
// геометрию поворота Аккермана, где внутренние и внешние колёса поворачиваются на разные углы,
// чтобы предотвратить проскальзывание шин. Передний привод с моторным моментом и торможением.
...
// Целевая скорость задаётся на основе значения ввода; момент прикладывается при ускорении.
...
// Угол поворота задаётся из ввода; мёртвая зона предотвращает дрейф от шума стика.
...
// Торможение симулируется путём применения высокого углового демпфирования к задним колёсам.
// Это сопротивляется вращению колёс, замедляя транспортное средство.
...
// Автомобиль сбрасывается в исходную позицию и ориентацию.
void Car::respawn()
{
	node->setTransform(translate(Vec3_up * 1.0) * rotate(Vec3_up, 90.0));
}

// Получаются шарниры колёс, настраивается камера преследования для слежения за автомобилем.
void Car::init()
{
	// Шарниры колёс получаются из тел узлов колёс
	if (wheelBl)
	...
	// Камера преследования настраивается для слежения за автомобилем
...
// Вычисляются и применяются к передним колёсам углы поворота Аккермана.
void Car::update()
{
	...
	// Геометрия поворота Аккермана: внутреннее и внешнее колёса поворачиваются на разные углы.
	// Это предотвращает проскальзывание колёс при повороте, заставляя все колёса вращаться вокруг
	// общей точки (центра поворота).
	float angle_0 = current_turn_angle;
	float angle_1 = current_turn_angle;
	if (Math::abs(current_turn_angle) > Consts::EPS)
	{
		// Радиус поворота вычисляется из угла поворота руля и колёсной базы
		float radius = carBase / Math::tan(current_turn_angle * Consts::DEG2RAD);

		// Внутреннее колесо имеет меньший радиус (более острый угол), внешнее - больший радиус
		float radius_0 = radius - carWidth * 0.5f;
		float radius_1 = radius + carWidth * 0.5f;

		// Радиусы преобразуются обратно в углы поворота
		angle_0 = Math::atan(carBase / radius_0) * Consts::RAD2DEG;
		angle_1 = Math::atan(carBase / radius_1) * Consts::RAD2DEG;
	}

	// Углы поворота применяются путём вращения осей шарниров колёс
	joint_wheel_fr->setAxis10(Math::rotateZ(angle_1).getColumn3(0));
	joint_wheel_fl->setAxis10(Math::rotateZ(angle_0).getColumn3(0));
}

// Параметры моторов колёс обновляются с фиксированным шагом физики.
void Car::updatePhysics()
{
	// Передний привод: целевая угловая скорость задаётся для передних колёс
	joint_wheel_fl->setAngularVelocity(current_velocity);
	joint_wheel_fr->setAngularVelocity(current_velocity);

	// Момент определяет, насколько сильно мотор может достичь целевой скорости
