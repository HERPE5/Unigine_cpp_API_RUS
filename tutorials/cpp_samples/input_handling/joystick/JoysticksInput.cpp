// Пример ввода с нескольких джойстиков с тестированием эффектов отдачи. Обрабатывает события
// горячего подключения/отключения. Отображает оси, переключатели POV, кнопки.
// Создаёт UI для тестирования эффектов FFB: постоянный, трение, пружина, волновые паттерны и т.д.
...
// Подписываемся на события горячего подключения и инициализируем структуры info для подключённых джойстиков.
void JoysticksInput::init()
{
	// Подписываемся на события горячего подключения для динамического подключения/отключения джойстиков
	...
	// Инициализируем структуры info для всех подключённых на данный момент джойстиков.
	// Каждый джойстик получает собственное хранилище данных эффектов FFB.
...
// Элементы управления UI для отдачи создаются для указанного столбца джойстика.
void JoysticksInput::create_ffb_ui(int info_column, Unigine::InputJoystickPtr &joystick)
{
	// UI отдачи пропускается, если джойстик не поддерживает никаких эффектов отдачи
...
// Столбцы UI обновляются, отображается состояние джойстика (оси, POV, кнопки).
void JoysticksInput::update()
{
	...
	for (int i = 0, column_index = 0; i < joysticks_info.size(); i++)
	{
		...
		// получаем информацию о джойстике
		...
		// заполняем информацию о джойстике
	...
	// Таймеры эффекта нарастания обновляются для автоматического переключения кнопок по истечении времени
	for (auto& info : joysticks_info)
		info.ramp_effect_data.update();
}

// Подключения событий освобождаются, информация о джойстиках очищается.
...
// Новая структура информации о джойстике создаётся при горячем подключении джойстика.
...
// Информация о джойстике удаляется при отключении джойстика.
...
// Панель UI для одного эффекта FFB создаётся со слайдерами и кнопкой воспроизведения/остановки.
...
// Эффект FFB перенаправляется в соответствующий метод джойстика в зависимости от типа эффекта.
...
// Активный эффект FFB останавливается на джойстике.
...
// Проверяются все типы эффектов FFB, чтобы определить, поддерживает ли джойстик хотя бы один.
...
// Таймер длительности обновляется; кнопка воспроизведения автоматически отключается по истечении эффекта.
