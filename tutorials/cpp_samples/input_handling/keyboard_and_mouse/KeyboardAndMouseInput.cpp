// Отображает состояние ввода с клавиатуры и мыши в реальном времени. Отслеживает переходы клавиш/кнопок
// (down, pressed, up), позицию мыши в пространстве экрана и окна, прокрутку колеса
// и сырые дельты мыши. Поддерживает события текстового ввода для захвата символов юникода.
...
void KeyboardAndMouseInput::init()
{
	...
	// GRAB - курсор скрыт и заблокирован в центре окна
	// SOFT - курсор видим, может свободно перемещаться
	// USER - приложение управляет видимостью курсора вручную
	...
	// Подписываемся на события текстового ввода для захвата вводимых символов (юникод)
...
void KeyboardAndMouseInput::update()
{
	...
	// Сбрасываем информацию о клавишах
	...
	// Сбрасываем координаты позиции окна
	...
	// Сбрасываем координаты позиции мыши в пространстве экрана
	...
	// Сбрасываем координаты позиции мыши в пространстве окна
	...
	// Сбрасываем значения колеса мыши
...
// Отслеживает переходы состояний кнопок мыши между кадрами.
// Down/Up срабатывают один раз при изменении состояния, Pressed срабатывает непрерывно при удержании.
void KeyboardAndMouseInput::check_last_mouse_button()
{
	for (int i = 0; i < Input::MOUSE_NUM_BUTTONS; i++)
	{
		...
		// Используем HashSet, чтобы отличить первое нажатие от продолжающегося удержания
...
// Вызывается для каждого введённого текстового символа (корректно обрабатывает юникод).
// Это отдельно от событий клавиш - захватывает фактически введённые символы,
// включая символы, требующие модификаторов (shift, alt), или ввод IME.
