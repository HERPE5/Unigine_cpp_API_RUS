...
// Создаёт виджет с включённой обводкой шрифта (для лучшей читаемости на 3D-фоне)
template<typename T>
...
// Создаёт виджет с включённой обводкой шрифта (перегрузка без параметра text)
template<typename T>
...
// Создаёт подписанное поле с меткой имени и виджетом, расположенными горизонтально
...
// Создаёт подписанное поле с меткой имени и двумя виджетами, расположенными горизонтально
...
///////////////////////////////////////////////////////////////////////////////////
// WidgetVec3 Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует виджет-редактор Vec3 с полями ввода и спинбоксами X, Y, Z
...
// Задаёт отображаемое значение во всех трёх полях ввода
...
// Возвращает текущее значение из всех трёх полей ввода как Vec3
...
///////////////////////////////////////////////////////////////////////////////////
// WidgetVec2 Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует виджет-редактор Vec2 с полями ввода и спинбоксами X, Y
...
// Задаёт отображаемое значение в обоих полях ввода
...
// Возвращает текущее значение из обоих полей ввода как Vec2
...
///////////////////////////////////////////////////////////////////////////////////
// BoneSelection Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует панель выбора костей с деревом всех костей скелета
void BoneSelection::init(const ObjectMeshSkinnedLegacyPtr &obj)
{
	skinned = obj;
...
// Возвращает индекс текущей выбранной кости в дереве, или -1, если не выбрана
...
// Создаёт и настраивает UI-виджеты выбора костей (group box, scroll box, дерево)
...
///////////////////////////////////////////////////////////////////////////////////
// LookAtEditor Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует редактор LookAt-цепочки со скелетным мешем и ссылкой на выбор костей
void LookAtEditor::init(ObjectMeshSkinnedLegacyPtr &obj, const BoneSelection *selection)
{
	if (main_group == nullptr)
		init_widgets();
...
// Возвращает ID текущей выбранной LookAt-цепочки, или -1, если не выбрана
...
// Создаёт все UI-виджеты редактора LookAt-цепочки (список цепочек, настройки, список костей)
void LookAtEditor::init_widgets()
{
	...
	chain_constraint_combo_box->getEventChanged().connect(widget_connections, [this]()
	{
		...
		skinned->setLookAtChainConstraint(static_cast<ObjectMeshSkinnedLegacy::CHAIN_CONSTRAINT>(chain_constraint_combo_box->getCurrentItem()), id);
	});
...
// Обновляет список LookAt-цепочек в UI
...
// Обновляет список костей в текущей выбранной цепочке
...
// Вызывается при изменении выбранной цепочки - обновляет UI и визуализацию
...
// Вызывается при изменении выбранной кости - обновляет настройки кости и визуализацию
...
///////////////////////////////////////////////////////////////////////////////////
// IKEditor Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует редактор IK-цепочки со скелетным мешем и ссылкой на выбор костей
void IKEditor::init(ObjectMeshSkinnedLegacyPtr &obj, const BoneSelection *selection)
{
	if (main_group == nullptr)
		init_widgets();
...
// Возвращает ID текущей выбранной IK-цепочки, или -1, если не выбрана
...
// Создаёт все UI-виджеты редактора IK-цепочки (список цепочек, настройки, список костей)
void IKEditor::init_widgets()
{
	...
	chain_constraint_combo_box->getEventChanged().connect(widget_connections, [this]()
	{
		...
		skinned->setIKChainConstraint(static_cast<ObjectMeshSkinnedLegacy::CHAIN_CONSTRAINT>(chain_constraint_combo_box->getCurrentItem()), id);
	});
...
// Обновляет список IK-цепочек в UI
...
// Обновляет список костей в текущей выбранной IK-цепочке
...
// Вызывается при изменении выбранной IK-цепочки - обновляет UI и визуализацию
...
// Вызывается при изменении выбранной кости - обновляет визуализацию кости
...
///////////////////////////////////////////////////////////////////////////////////
// ConstraintEditor Implementation
///////////////////////////////////////////////////////////////////////////////////

// Инициализирует редактор ограничений со скелетным мешем и ссылкой на выбор костей
void ConstraintEditor::init(ObjectMeshSkinnedLegacyPtr &obj, const BoneSelection *selection)
{
	if (main_group == nullptr)
		init_widgets();
...
// Создаёт все UI-виджеты редактора ограничений (список ограничений, настройки)
...
// Обновляет список ограничений костей в UI
...
// Вызывается при изменении выбранного ограничения - обновляет UI и визуализацию
