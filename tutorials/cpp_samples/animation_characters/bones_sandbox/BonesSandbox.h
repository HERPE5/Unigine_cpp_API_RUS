...
// Пользовательский виджет для редактирования значений Vec3 с помощью трёх спинбоксов (X, Y, Z)
...
// Пользовательский виджет для редактирования значений Vec2 с помощью двух спинбоксов (X, Y)
...
// Виджет, отображающий иерархию костей в виде дерева, позволяющий выбирать кости.
// Выбранная кость подсвечивается в визуализации вьюпорта.
...
public:
	...
	void init(const Unigine::ObjectMeshSkinnedLegacyPtr &obj);
...
// Панель редактора для создания и настройки LookAt-цепочек.
// Позволяет добавлять/удалять цепочки и кости, задавать позиции цели/полюса,
// а также настраивать веса и оси для каждой кости.
...
public:
	...
	void init(Unigine::ObjectMeshSkinnedLegacyPtr &obj, const BoneSelection *selection);
...
// Панель редактора для создания и настройки IK-цепочек (обратной кинематики).
// Предоставляет элементы управления для позиций цели/полюса, вращения эффектора, итераций решателя,
// допуска и режимов ограничений.
...
// Панель редактора для создания и настройки ограничений вращения костей.
// Позволяет задавать ограничения вращения (мин/макс углы) по осям yaw, pitch и roll
// для отдельных костей.
