...
void AnimationBonesSandboxSample::init()
{
	...
	// Включаем визуализацию скелета для всех костей
	skinned->setVisualizeAllBones(true);

	// Создаём пример LookAt-цепочки от позвоночника до головы.
	// Веса увеличиваются от позвоночника (0.05) до головы (1.0) для естественного распределения взгляда.
	{
	...
	// Создаём пример IK-цепочки для левой ноги (бедро -> голень -> стопа)
	{
	...
	// Создаём пример ограничения вращения для кости головы (ограничивает диапазон вращения головы)
	{
	...
	// Инициализируем и добавляем панели редактора
	...
	// Создаём отдельное окно для редакторов
