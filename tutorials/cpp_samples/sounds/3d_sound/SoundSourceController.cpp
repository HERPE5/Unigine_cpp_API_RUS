// Демонстрирует 3D-позиционный звук с помощью SoundSource. Звук пространственно позиционируется
// в зависимости от расстояния до слушателя с настраиваемым мин./макс. затуханием по расстоянию. Поддерживает
// потоковый режим для больших файлов и визуализирует местоположение источника звука.
...
// Создаётся 3D-источник звука, настраиваются элементы управления UI.
void SoundSourceController::init()
{
	...
	// Создаём позиционный источник звука из файла
	...
	// Настраиваем затухание по расстоянию: полная громкость на 5м, тишина на 50м
...
// Визуализируются сферы расстояния источника звука.
