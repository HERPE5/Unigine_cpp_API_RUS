// Демонстрирует воспроизведение окружающего (непозиционного) звука. AmbientSource воспроизводит аудио
// без 3D-позиционирования - идеально для музыки или звуков UI. Поддерживает переключение
// между потоковым и буферизованным режимами во время выполнения с сохранением состояния воспроизведения.
...
// AmbientSource создаётся из файла, настраиваются элементы управления UI.
void SoundAmbient::init()
{
	...
	// Создаём непозиционный источник звука из файла
	...
	// Запускаем зацикленное воспроизведение на полной громкости
...
// Источник аудио освобождается, UI очищается.
...
// Источник пересоздаётся с новым режимом потока с сохранением состояния воспроизведения.
void SoundAmbient::change_souce_type()
{
	// Захватываем текущие настройки перед уничтожением источника
	...
	// Переключаем режим потока и пересоздаём источник
	is_stream = !is_stream;
	if (is_stream)
	...
	// Восстанавливаем предыдущие настройки для нового источника
